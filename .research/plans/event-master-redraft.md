---
name: event-master
description: "Draft rewrite of the Cerebral Valley event umbrella skill. Use this skill when working on CV event operations in this workspace: find the right event docs and templates, resolve the relevant workflow nodes and adjacent steps, verify live state in the platform DB, use local Granola/Slack/Discord context when helpful, and then actually execute the next artifact or update instead of stopping at research."
---

# Event Master Redraft

Draft only. Do not treat this file as the live installed skill.

This draft keeps `event-master` detailed and command-rich, but reorganizes it around:
- orchestration
- shared surfaces
...
1. Read this draft for the orchestration rules and task-family routing.
...
Draft-note:
- this redraft is not a standalone installed skill yet
- when it references `references/*.md` or `scripts/...`, it assumes the live installed `event-master` reference set and scripts
- before promotion, those links/paths should remain aligned with the installed skill tree
